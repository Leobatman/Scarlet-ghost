...
# ============= NOVA FUNÇÃO: DEPENDÊNCIAS DO NAABU =============
install_naabu_dependencies() {
    log "INFO" "Instalando dependências do naabu (libpcap)..."
    ...
    case "$manager" in
        apt|apt-get)
            ...
            sudo apt-get install -y libpcap-dev libpcap0.8-dev
            ;;
        dnf|yum)
            sudo $manager install -y libpcap-devel
            ;;
        pacman)
        ...
        brew)
            ...
            ;;
        *)
            log "ERROR" "Não foi possível instalar libpcap automaticamente"
            log "INFO" "Instale libpcap-dev manualmente e tente novamente"
            return 1
            ;;
    esac
    
    # Verificar instalação
    if find /usr/include /usr/local/include -name "pcap.h" 2>/dev/null | grep -q .; then
...
# ============= FUNÇÃO MODIFICADA: NUCLEI SUITE COM DEPENDÊNCIAS =============
install_nuclei_suite_enhanced() {
    log "INFO" "Instalando Nuclei Suite..."
    
    install_golang
    
    # Instalar dependências do naabu ANTES de tentar compilar
    log "INFO" "Verificando dependências para ferramentas de rede..."
    install_naabu_dependencies || log "WARN" "Algumas dependências podem estar faltando"
